...
function contractIdOf(event: rpc.Api.EventResponse): string {
  const raw: unknown = (event as { contractId?: unknown } | null)?.contractId;
...
export function decodeEvent(source: ContractSource, event: rpc.Api.EventResponse): DecodedEvent {
  ...
  let eventName = "";
  try {
    const rawTopic: unknown = (event as { topic?: unknown } | null)?.topic ?? [];
    const topicArray = Array.isArray(rawTopic) ? rawTopic : [];
    const topics = topicArray.map((t) => {
      try {
    ...
    const decodedValue = native((event as { value?: xdr.ScVal }).value as xdr.ScVal);
