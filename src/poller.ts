...
export interface PollerDeps {
  ...
  /** Override the chain reader in tests without coupling them to the RPC SDK. */
  readEvents?: typeof readContractEvents;
  /** Telegram pacing interval; production defaults to the safe chat limit. */
...
export function createPoller(deps: PollerDeps) {
  ...
  async function notify(events: DecodedEvent[], delivery: { sent: number }): Promise<void> {
    for (const event of events) {
      ...
      if (delivery.sent < config.maxNotificationsPerCycle && sendSpacingMs > 0) {
  ...
  return {
    ...
    /** Run one bounded poll cycle; exposed for deterministic integration tests. */
    pollOnce: cycle,
  };
}
